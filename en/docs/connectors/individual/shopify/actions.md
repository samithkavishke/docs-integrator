---
title: "Shopify Admin - Actions"
description: "Available actions and operations for the ballerinax/shopify.admin connector."
---

# Shopify Admin Actions

The `ballerinax/shopify.admin` package provides a client with operations to interact with Shopify Admin.

## Client Initialization

```ballerina
import ballerinax/shopify.admin;
import ballerina/log;

configurable string token = ?;

final admin:Client adminClient = check new ({
    auth: {
        token: token
    }
});
```

## Common Operations

For the complete list of operations and their parameters, see the [API documentation](https://central.ballerina.io/ballerinax/shopify.admin/latest#clients).

### Create

```ballerina
public function createRecord() returns error? {
    var result = check adminClient->create({
        // Record fields
    });
    log:printInfo("Created successfully", id = result.id);
}
```

### Read

```ballerina
public function getRecord(string id) returns error? {
    var result = check adminClient->get(id);
    log:printInfo("Retrieved record", data = result);
}
```

### Update

```ballerina
public function updateRecord(string id) returns error? {
    var result = check adminClient->update(id, {
        // Updated fields
    });
    log:printInfo("Updated successfully");
}
```

### Delete

```ballerina
public function deleteRecord(string id) returns error? {
    check adminClient->delete(id);
    log:printInfo("Deleted successfully");
}
```

## Error Handling

```ballerina
do {
    // connector operation
} on fail error e {
    log:printError("Operation failed", 'error = e);
}
```

## Related

- [Overview](overview)
- [Setup Guide](setup)
- [Examples](examples)
