---
title: "Milvus Vector Database - Examples"
description: "Code examples for the ballerinax/milvus connector."
---
...
public function main() returns error? {
    // Initialize client
    milvus:Client client = check new ({
        auth: { token: token }
    });
    
    io:println("Connected to Milvus Vector Database successfully");
}
```
...
service /api on new http:Listener(9090) {
    ...
    resource function get data() returns json|error {
        ...
        return { "status": "ok", "source": "Milvus Vector Database" };
    }

    resource function post data(json payload) returns http:Created|error {
        // Use the connector to send data
        log:printInfo("Sending data to Milvus Vector Database");
        return http:CREATED;
    }
}
```
